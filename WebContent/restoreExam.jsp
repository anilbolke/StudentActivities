...
<%
    ...
    if (examId > 0) {
        boolean restored = ExamDAO.updateExamStatus(examId, "PUBLISHED");
        if (restored) {
            response.sendRedirect("dashboard.jsp?message=Exam restored successfully");
        } else {
            response.sendRedirect("dashboard.jsp?error=Failed to restore exam");
        }
    } else {
